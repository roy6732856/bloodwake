import {buildUpgrades} from '../src/builds.js';
import {animateHunter} from '../src/hunter-model.js';
...
button.onclick=async()=>{
  ...
  test('XP pickup pauses for 3 unique upgrades; selection resumes',()=>{clearEnemies();g.dropGem(0,0,12);step(.1);assert(g.state==='upgrade','XP did not trigger level');assert(doc.querySelectorAll('[data-choice]').length===3,'missing 3 UI choices');assert(new Set(g.pendingChoices.map(u=>u.id)).size===3,'duplicate choices');g.chooseUpgrade(0);assert(g.state==='playing'&&g.upgradesChosen===1,'upgrade did not resume');});
  ...
  test('Artist-authored skinned bodies animate independently and role changes retain intact heads',()=>{
    for(const role of ['hunter','shade','oracle','pyre','sentinel']){g.lobby.select(role,false);g.start({hunter:role});const actor=g.hunter.userData.variants[role],menuActor=g.lobby.model.userData.variants[role];let skins=0;actor.traverse(o=>{if(o.isSkinnedMesh)skins++;});assert(skins>=4,'missing skinned clothing');assert(actor.getObjectByName('Head').children.some(x=>x.name.startsWith('Portrait_')),'missing real head');assert(actor.getObjectByName('pelvis')!==menuActor.getObjectByName('pelvis'),'shared skeleton between lobby and combat');g.hunter.userData.moving=true;animateHunter(g.hunter,1);animateHunter(g.hunter,1.04);g.renderer.render(g.scene,g.camera);assert(g.renderer.info.render.triangles>1000,'model did not render');}
  });
  test('All autonomous main skills damage actual targets and clean up on restart',()=>{
    for(const id of ['storm','ember','scythe','comet']){g.start({director:'classic'});clearEnemies();g.ranks[id]=5;g.aimDirection.set(0,0,-1);const e=g.spawnEnemy('brute',0,-2);e.speed=0;e.hp=e.maxHp=10000;assert(g.arsenal.cast(id),'skill failed to cast');for(let i=0;i<150;i++)g.arsenal.update(1/60);assert(e.hp<e.maxHp,id+' never damaged');g.pause();const count=g.arsenal.zones.map(z=>z.life).join();g.arsenal.update(2);assert(g.arsenal.zones.map(z=>z.life).join()===count,'paused zones advanced');g.start();assert(g.arsenal.zones.length===0&&g.arsenal.missiles.length===0,'stale skill effects after restart');}
  });
  test('Evolutions change geometry and target patterns, not only damage numbers',()=>{
    g.start({director:'classic'});clearEnemies();g.ranks={armament:1,ember:5,regen:1};for(let i=0;i<3;i++){const e=g.spawnEnemy('brute',i*3,-4);e.hp=10000;e.speed=0;}g.checkEvolution();g.arsenal.cast('ember');assert(g.arsenal.zones.length===3,'evolved fire bottle did not make three zones');
    g.start({director:'classic'});clearEnemies();g.ranks={armament:1,comet:5,damage:1};g.spawnEnemy('brute',0,-5);g.checkEvolution();g.arsenal.cast('comet');assert(g.arsenal.missiles.length===3&&g.arsenal.missiles.every(m=>m.evolved),'comet evolution did not change volley/explosion');
    g.start({director:'classic'});clearEnemies();g.ranks={armament:1,scythe:5};g.aimDirection.set(0,0,-1);const rear=g.spawnEnemy('brute',0,2);rear.hp=rear.maxHp=10000;g.arsenal.cast('scythe');assert(rear.hp===rear.maxHp,'base arc hit behind player');g.ranks.health=1;g.hp=50;g.checkEvolution();g.arsenal.cast('scythe');assert(rear.hp<rear.maxHp&&g.hp>50,'evolved arc missing full circle or healing');
    g.start({director:'classic'});clearEnemies();g.ranks={armament:1,storm:5,wisdom:1};const targets=[];for(let i=0;i<7;i++){const e=g.spawnEnemy('brute',i*.7,-4);e.hp=e.maxHp=10000;targets.push(e);}g.checkEvolution();g.arsenal.cast('storm');assert(targets.every(e=>e.hp<e.maxHp),'evolved storm did not chain through seven targets');
    g.start();g.ranks={armament:1,orbit:5,magnet:1};g.stats.orbit=5;g.checkEvolution();g.encounters.update(.01);assert(g.stats.orbit===7&&g.encounters.blades.some(b=>b.position.distanceTo(g.hunter.position)>3),'evolved blades missing outer orbit');assert(!g.checkEvolution()&&g.stats.orbit===7,'evolution applied twice');
  });
  try{
    g.start();clearEnemies();g.mouseDown=true;g.gainXp(12);const options=g.pendingChoices,chosen=options[0],before=g.ranks[chosen.id]||0;
    assert(g.state==='upgrade'&&!g.mouseDown,'shooting not released');assert(doc.querySelector('#confirm-upgrade').disabled,'confirmation was armed without selection');
    doc.querySelector('[data-choice="0"]').click();doc.querySelector('[data-choice="0"]').click();assert(g.state==='upgrade'&&(g.ranks[chosen.id]||0)===before,'repeated card clicks committed upgrade');assert(!g.confirmUpgrade(),'confirmation guard failed');
    await new Promise(r=>setTimeout(r,210));g.xp=100;doc.querySelector('#confirm-upgrade').click();assert(g.ranks[chosen.id]===before+1,'confirmation did not apply exactly once');assert(g.state==='upgrade'&&g.selectedUpgrade===-1&&doc.querySelector('#confirm-upgrade').disabled,'queued level inherited old selection');
    doc.querySelector('[data-choice="0"]').click();g.reroll();assert(g.selectedUpgrade===-1&&doc.querySelector('#confirm-upgrade').disabled,'reroll retained selected card');lines.push('PASS Upgrade requires selection plus confirmation; repeated clicks, queued levels and rerolls are safe');
  }catch(e){lines.push('FAIL Upgrade confirmation: '+e.message);}
  try {
