import {upgradeMarkup,selectUpgradeUI,inventoryMarkup,buildCodexMarkup} from './upgrade-ui.js';
import {loadHunterAssets} from './hunter-model.js';
import {maps,missions} from './world-content.js';
import { recordRun, rateRun, recommend, trials } from './bloodmoon.js';
...
let selection=normalizeLoadout({mission:'hunt'}),hitTimer,evolutionTimer,cloud;
...
function refreshExpedition(){ $('open-expedition').textContent=`${maps.find(m=>m.id===selection.map).name} · ${missions.find(m=>m.id===selection.mission).name} ↗`; }
function showLoadout(focus=true){refreshExpedition();showDialog('loadout',loadoutMarkup(selection,save),focus);for(const kind of ['hunter','weapon','contract','director','trial','map','mission'])document.querySelectorAll(`[data-select-${kind}]`).forEach(b=>b.onclick=()=>{const scroll=$('dialog-content').scrollTop;selection[kind]=b.getAttribute(`data-select-${kind}`);if(kind==='hunter')game.lobby.select(selection.hunter,false);else if(kind==='weapon')game.lobby.applyDye();showLoadout(false);$('dialog-content').scrollTop=scroll;document.querySelector(`[data-select-${kind}="${selection[kind]}"]`)?.focus({preventScroll:true});});refreshExpedition();$('deploy').onclick=()=>game.start(selection);$('loadout-back').onclick=()=>{closeDialog();refreshExpedition();};}
function showCodex(){showDialog('codex',codexMarkup(save).replace('<div class="dialog-actions">',buildCodexMarkup(weapons.find(w=>w.id===selection.weapon))+'<div class="dialog-actions">'));$('codex-back').onclick=closeDialog;}
function showHelp(){showDialog('help',`<div class="sub">獵人手冊</div><h2>活到黎明</h2><p>在血月獵場存活 5 分鐘，並完成所選任務。<br>追獵需擊倒三名標記首領；儀式需依序淨化三座祭壇。<br>擊倒敵人、拾取青色靈魂，<br>升級時從三項隨機強化中選擇一項。</p><dl class="key-list"><dt>W A S D</dt><dd>移動，也支援方向鍵</dd><dt>滑鼠 + 左鍵</dt><dd>瞄準地面位置；按住左鍵持續射擊</dd><dt>SPACE</dt><dd>朝移動方向衝刺，短暫無敵</dd><dt>E</dt><dd>釋放新星，傷害並擊退周圍敵人</dd><dt>Q</dt><dd>開啟附近聖匣，獲得額外強化</dd><dt>F</dt><dd>切換自動射擊，仍由滑鼠瞄準</dd><dt>ESC / P</dt><dd>暫停與繼續；切換視窗自動暫停</dd><dt>1 / 2 / 3</dt><dd>選取升級卡片，再按 ENTER 確認</dd><dt>觸控裝置</dt><dd>左搖桿移動，右搖桿瞄準並射擊；點技能施放</dd></dl><p class="muted">紅色結晶恢復生命。墓碑會擋住銀彈，衝刺可閃避典獄長的環形衝擊波。<br>結算獲得的殘魂，可在「獵人傳承」換取永久能力。<br>血月意志會調整敵潮節奏；金色裂隙預告包抄。結算回饋會影響下局推薦。</p><button id="help-back" class="primary">返回</button>`);$('help-back').onclick=closeDialog;}
function showLegacy(){refreshMenu();showDialog('legacy',`<div class="sub">跨局永久成長</div><h2>獵人傳承</h2><p>可用殘魂 <strong>${save.souls}</strong>　·　每項最高 5 階</p>${legacy.map(u=>{const r=save.ranks[u.id],cost=legacyCost(r);return `<div class="legacy-row"><span class="symbol">${u.symbol}</span><div><h3>${u.name} <small>${r} / 5</small></h3><p>${u.description}</p><small class="legacy-next">${u.id==='power'?`目前傷害 +${r*8}% → 下階 +${Math.min(5,r+1)*8}%`:u.id==='vitality'?`目前生命 +${r*10} → 下階 +${Math.min(5,r+1)*10}`:`目前拾取 +${(r*.5).toFixed(1)}m → 下階 +${(Math.min(5,r+1)*.5).toFixed(1)}m`}</small></div><button data-buy="${u.id}" ${r>=5||save.souls<cost?'disabled':''}>${r>=5?'已達上限':`${cost} 殘魂 · 強化`}</button></div>`;}).join('')}<p class="muted" style="margin-top:22px">能力在下一局開始時套用。${storageAvailable?'進度自動儲存在此瀏覽器，可在主選單開啟雲端同步。':'瀏覽器儲存不可用，進度只保留於本次開啟期間。'}</p><div class="dialog-actions"><button id="legacy-back" class="primary">返回獵場</button></div>`);$('legacy-back').onclick=()=>{closeDialog();refreshMenu();};document.querySelectorAll('[data-buy]').forEach(b=>b.onclick=()=>{if(purchase(save,b.dataset.buy)){persist();showLegacy();}});}
function showPause(){showDialog('pause',`<div class="sub">${formatTime(game.time)} · LV ${game.level}</div><h2>夜色暫歇</h2><p>獵場已暫停，準備好再繼續。</p>${inventoryMarkup(game)}<p class="muted">WASD 移動　·　左鍵射擊　·　SPACE 衝刺　·　E 新星<br>F 自動射擊　·　ESC 返回遊戲</p><div class="dialog-actions"><button id="resume" class="primary">繼續狩獵</button><button id="abandon">結束本局</button></div>`);$('resume').onclick=()=>game.resume();$('abandon').onclick=()=>{game.state='playing';game.finish(false,true);};}
function showUpgrade(options){$('reticle').classList.add('hidden');showDialog('upgrade',upgradeMarkup(game,options));document.querySelectorAll('[data-choice]').forEach(b=>b.onclick=()=>game.selectUpgrade(Number(b.dataset.choice)));$('confirm-upgrade').onclick=()=>game.confirmUpgrade();$('reroll').onclick=()=>game.reroll();}

function showEnd(result){
  const earned=settleRun(save,result),unlocked=unlockAchievements(save,result);recordRun(save,result);persist();refreshMenu();$('reticle').classList.add('hidden');
  showDialog('end',`<div class="sub">${result.win?'黎明終於到來':'下一位獵人，將繼承你的意志'}</div><h2>${result.win?'血月落幕':'長夜未盡'}</h2><p>${result.win?'你完成了這個夜晚的任務。':result.time>=300?'時間已到，但任務目標尚未完成。':'銀彈耗不盡黑夜，但留下的靈魂不會消失。'}</p><div class="result-stats"><div><strong>${formatTime(result.time)}</strong><span>存活時間</span></div><div><strong>${result.kills}</strong><span>擊殺數</span></div><div><strong>+${earned}</strong><span>獲得殘魂</span></div></div><p class="muted">每 8 次擊殺 +1 殘魂 · 每存活 20 秒 +1 · 生還額外 +20<br>目前共有 ${save.souls} 殘魂，可在主選單「獵人傳承」購買永久強化。</p><div class="dialog-actions"><button id="retry" class="primary">再次狩獵</button><button id="end-legacy">用殘魂強化</button><button id="back-menu">返回主選單</button></div>`);
  $('end-legacy').onclick=()=>{game.toMenu();showLegacy();};$('retry').onclick=()=>game.start();$('back-menu').onclick=()=>game.toMenu();
  const details=document.createElement('div');details.className='result-details';details.innerHTML=`<p>${maps.find(m=>m.id===result.map)?.name||'血月墓園'} · ${missions.find(m=>m.id===result.mission)?.name||'自由生存'}${result.mission!=='survival'?' · 目標 '+(result.objective||0)+' / 3':''}</p><p>${result.weapon} ${result.evolved?'· 已進化':''}　／　最高 ${result.bestCombo||0} 連殺　／　開啟 ${result.chests||0} 聖匣</p>${unlocked.map(a=>`<div class="unlock-notice">✦ ${a.name} 解鎖　+${a.reward} 殘魂</div>`).join('')}<small>契約與試煉收益 ×${(game.contract.reward*(trials.find(t=>t.id===game.loadout.trial)?.reward||1)*(missions.find(m=>m.id===game.loadout.mission)?.reward||1)).toFixed(2)} · 目前 ${save.souls} 殘魂</small>`;$('dialog-content').querySelector('.dialog-actions').before(details);
  const recap=document.createElement('div');recap.className='moon-recap';recap.innerHTML=`<div class="sub">血月記憶</div><p>${result.mode==='classic'?'經典節奏':`本局 ${result.director?.peaks||0} 次獵潮 · ${result.director?.breaths||0} 次餘息`} · ${trials.find(t=>t.id===result.trial)?.name||'自由狩獵'}</p><p>這一局的挑戰感如何？</p><div class="rating-buttons">${[['easy','太輕鬆'],['fair','剛剛好'],['hard','太挫折']].map(([id,label])=>`<button data-rating="${id}" aria-pressed="false">${label}</button>`).join('')}</div><p id="rating-note">${result.abandoned?'主動結束的戰局不計入推薦。':'你的感受會影響下一局的推薦，回饋可更改。'}</p><button id="prepare-next">查看推薦 · 調整下一局</button>`;details.after(recap);
  recap.querySelectorAll('[data-rating]').forEach(b=>b.onclick=()=>{if(rateRun(save,result.runId,b.dataset.rating)){persist();recap.querySelectorAll('[data-rating]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));$('rating-note').textContent='已記住你的感受。'+recommend(save.history).reason;}});
  $('prepare-next').onclick=()=>{game.toMenu();showLoadout();};
}
...
function updateHud(s){
  ...
  const ex=s.expedition,objective=game.loadout.mission==='survival'?'存活至黎明':`${ex.progress} / 3 ${game.loadout.mission==='hunt'?'標記首領':'封印祭壇'}`;$('expedition-hud').textContent=`${ex.map} · ${ex.mission}｜${objective}${ex.target?` · ${Math.ceil(ex.target.distance)}m ${Math.abs(ex.target.dx)>Math.abs(ex.target.dz)?ex.target.dx>0?'→':'←':ex.target.dz>0?'↓':'↑'} · ${ex.blocked?'清除祭壇附近敵人':ex.charge.toFixed(1)+' / 8s'}`:''}${ex.shieldMax?`｜護盾 ${Math.ceil(ex.shield)} / ${ex.shieldMax}`:''}${ex.terrain==='warning'?'｜地形即將啟動':ex.terrain==='active'?game.loadout.map==='foundry'?'｜熔流噴發：走橋面':'｜寒霜生效：離開藍色區':''}`;
  const moon=$('moon-director');moon.dataset.phase=s.director.enabled?s.director.phase:'classic';moon.querySelector('strong').textContent=s.director.name;moon.querySelector('small').textContent=s.director.trial;moon.querySelector('i').style.width=`${s.director.pressure*100}%`;
  ...
  const recipe=game.weapon.evolution.requires,short={armament:'主武器',damage:'銀彈',rate:'扳機',spread:'獠牙',health:'契約',pierce:'穿透',crit:'暴擊'};
...
try{
  const loading=document.createElement('div');loading.id='model-loading';loading.setAttribute('role','status');loading.textContent='正在載入角色與動作…';document.body.append(loading);
  await loadHunterAssets();loading.remove();
  game=new Game($('world'),save,emit);
  game.lobby=new Lobby(game,selection,()=>{});
  $('quick-start').onclick=()=>game.start(selection);$('open-expedition').onclick=()=>{showLoadout();document.querySelector('[data-select-map]').scrollIntoView({block:'center'});};refreshExpedition();
...
}catch(error){document.getElementById('model-loading')?.remove();console.error(error);$('fatal').classList.remove('hidden');$('fatal-message').textContent='角色素材或 3D 畫面載入失敗。請確認網路連線及瀏覽器硬體加速後重新載入。';}
