...
function persist(){try{localStorage.setItem(storageKey,JSON.stringify(save));}catch{storageAvailable=false;}}
...
function showUpgrade(options){$('reticle').classList.add('hidden');showDialog('upgrade',`<div class="sub">靈魂覺醒 · LEVEL ${game.level}</div><h2>選擇你的力量</h2><p>時間已暫停。選一項強化，繼續狩獵。</p><div class="choice-grid">${options.map((u,i)=>`<button class="choice" data-choice="${i}"><span class="symbol">${u.symbol}</span><span class="type">${u.type}</span><h3>${u.name}</h3><p>${u.description}</p><small>按 ${i+1} 選擇　·　第 ${(game.ranks[u.id]||0)+1} 階</small></button>`).join('')}</div><span class="muted">本局強化可疊加，打造你的專屬流派。</span>`);document.querySelectorAll('[data-choice]').forEach(b=>b.onclick=()=>game.chooseUpgrade(Number(b.dataset.choice)));}
function showEnd(result){
  const earned=settleRun(save,result);persist();refreshMenu();$('reticle').classList.add('hidden');
...
function onState(state){
  ...
  if(state==='menu'){closeDialog();$('menu').classList.remove('hidden');$('hud').classList.add('hidden');refreshMenu();$('start').focus();}
}
...
function emit(kind,data){if(kind==='state')onState(data);if(kind==='frame')updateHud(data);if(kind==='upgrade')showUpgrade(data);if(kind==='end')showEnd(data);if(kind==='toast')toast(data);if(kind==='pointer'){$('reticle').style.left=`${data.x}px`;$('reticle').style.top=`${data.y}px`;}}
...
document.addEventListener('keydown',e=>{
  if(e.code==='Escape'&&['help','legacy'].includes(dialogKind))closeDialog();
  if(e.key==='Tab'&&!$('dialog').classList.contains('hidden')){const buttons=[...$('dialog-content').querySelectorAll('button:not(:disabled)')];const first=buttons[0],last=buttons.at(-1);if(e.shiftKey&&document.activeElement===first){e.preventDefault();last?.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first?.focus();}}
});
try{
  game=new Game($('world'),save,emit);
  $('start').onclick=()=>game.start();$('pause').onclick=()=>game.pause();$('dash').onclick=()=>game.dash();$('nova').onclick=()=>game.nova();
  ...
  $('open-help').onclick=showHelp;$('open-legacy').onclick=showLegacy;attachPad('move-pad',game.touchMove);attachPad('aim-pad',game.touchAim);refreshMenu();
